...
DEFAULT_MODEL_REPO_ID = "RobinWZQ/cliptrace-2026-models"
...
def default_encoder_path(phase, model_id):
    return PROJECT_ROOT / "cliptrace-2026-models" / "models" / phase / model_id


def is_huggingface_checkpoint(path):
    weight_files = (
        "model.safetensors",
...
def download_huggingface_model(encoder_path, repo_id, phase, model_id, token=None):
    """Download only the requested competition model into the expected local tree."""
    try:
    ...
    local_dir = PROJECT_ROOT / "cliptrace-2026-models"
    print(f"Model checkpoint not found at {encoder_path}.")
    print(f"Downloading {repo_id}:models/{phase}/{model_id} from Hugging Face...")
    try:
        snapshot_download(
            repo_id=repo_id,
            repo_type="model",
            local_dir=local_dir,
            allow_patterns=[f"models/{phase}/{model_id}/**"],
            token=token,
        )
    except Exception as exc:
        raise RuntimeError(
            f"could not download {repo_id}:models/{phase}/{model_id}. "
            "Accept the repository terms and run `hf auth login`, or set HF_TOKEN."
        ) from exc

    if not is_huggingface_checkpoint(encoder_path):
        raise FileNotFoundError(
            f"download completed but no complete Hugging Face checkpoint was found at {encoder_path}"
        )
    return encoder_path


def resolve_encoder_path(args):
    if args.encoder_path is None:
        model_id = args.model_id or DEFAULT_MODEL_ID
        encoder_path = default_encoder_path(args.phase, model_id)
    else:
        encoder_path = project_path(args.encoder_path)
        model_id = args.model_id or encoder_path.name

    if is_huggingface_checkpoint(encoder_path):
        return encoder_path
    if args.encoder_path is not None:
        raise FileNotFoundError(f"model checkpoint not found at explicit --encoder_path {encoder_path}")
    if not args.auto_download_model:
        raise FileNotFoundError(
            f"model checkpoint not found at {encoder_path}; remove "
            "--no-auto_download_model to download it automatically"
        )
...
def main(args):
    ...
    encoder_path = resolve_encoder_path(args)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help=f"model id to download/process; defaults to {DEFAULT_MODEL_ID} or the encoder directory name",
    )
    parser.add_argument(
        "--encoder_path",
        default=None,
        type=str,
        help=(
            "relative or absolute path to a CLIP model; defaults to "
            "cliptrace-2026-models/models/<phase>/<model_id>"
        ),
    )
    parser.add_argument(
        "--phase",
        ...
        help="competition phase used to locate or download the model",
    )
    parser.add_argument(
        "--model_repo_id",
        default=DEFAULT_MODEL_REPO_ID,
        help="Hugging Face repository containing the competition models",
    )
    parser.add_argument(
        "--auto_download_model",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="download the selected model from Hugging Face when it is missing locally",
    )
